...
export function useOccurrences() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Fetch 3 tables in parallel (Admin, Enf, Laudo) - REMOVED General/Patient which was legacy
...
export function useCreateOccurrence() {
  ...
  return useMutation({
    mutationFn: async (data: any) => {
      ...
      // Medical / Laudo (revisao_exame) - NOW GOES TO ADMIN TABLE AS REQUESTED
      if (data.tipo === 'revisao_exame' || data.subtipo === 'revisao_exame') {
        const payload = {
          tenant_id: profile.tenant_id,
          criado_por: profile.id,
          tipo: 'assistencial',
          subtipo: 'revisao_exame',
          titulo: `Revisão de Laudo - ${paciente.nomeCompleto}`,
          descricao: data.descricao_detalhada || JSON.stringify(data.dados_especificos || {}),
          ...
          paciente_unidade_local: data.paciente_unidade_local || data.unidadeLocal,
          ...
          medico_destino: (data.dados_especificos as any)?.medicoResponsavel || (data.dados_especificos as any)?.medicoAvaliou,
        };

        const { data: res, error } = await (supabase
          .from("ocorrencias_adm" as any) as any) // Cast to any to avoid strict typing issues with new columns
          .insert(payload)
          .select()
          .single();
        if (error) throw error;
